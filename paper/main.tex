...
All curves report the interquartile mean (IQM) with shaded standard error, and seed counts vary by experiment and are stated in each caption. The recurrence is not held constant, and each benchmark uses the cell from the work it comes from, minGRU for MemoryChain and \gls{rtu} elsewhere, chosen from the source rather than from performance on the benchmark. The choice still matters, since minGRU reaches longer chains on MemoryChain, where its real positive retentions accumulate coherently and \gls{rtu}'s complex rotation cancels, the same effect Section~\ref{sec:weights} measures. Appendix~\ref{sec:popgym-cells} repeats POPGym with the other two.
...
matters, since at $\lambda{=}1$ and $L{=}128$ a minGRU solves $60\%$ of seeds where an \gls{rtu}
solves none. Figures~\ref{fig:popgym-mingru}
