...
\begin{itemize}
    ...
    feedforward work, and we show why: the rules assume the trace is a decayed sum of recent
...
time, and the reach of the update is bounded by whichever decays more slowly. The bound governs how
...
\end{equation}
for $\rho \neq \gamma\lambda$; at equality the sum has $T-k+1$ terms of equal size and the
bound carries an additional factor $T-k+1$.
\end{proposition}
...
$0.2\%$ in every configuration, while the \gls{rtrl} weight does not follow $\gamma\lambda$:
dropping $\lambda$ from $1$ to $0.98$ moves the fitted TBPTT(1) rate from $0.999$ to $0.979$ and
leaves the \gls{rtrl} rate at $0.994$ or above. Over $32$ lags that rate sits at or just above
one, the $n+1$ prefactor of the near-degenerate regime $\rho \approx \gamma\lambda$ rather than
a decay, so the bound orders the two channels without being tight on either. The amplification
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.76\linewidth]{plots/mujoco/learning_curves.pdf}
...
benchmark (Figure~\ref{fig:brax}). Bootstrapping the difference of means over the 10 seeds per cell separates only three of eight: \gls{ppo} is ahead on
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.80\linewidth]{plots/foragax/foragax_body.pdf}
...
with matters more than the recurrence itself: \gls{rtu} costs stream RQ($\lambda$) nothing,
...
corrected, and on Foragax the best agent under that update is memoryless. Retention is necessary but not sufficient: on MemoryChain the
credit horizon exceeds the solvable chain everywhere. Two limitations are
...
\begin{table}[H]
\centering
\caption{Decay of the credit weight $\|\boldsymbol{\omega}_k\|$ with lag on trained MemoryChain agents ($L{=}64$, medians over $10$ seeds) against the rates of Proposition~\ref{prop:composition}: $\gamma\lambda$ for TBPTT(1) and $\max(\gamma\lambda, \rho)$ for RTRL, with $\rho$ the measured retention. The TBPTT(1) rate follows $\gamma\lambda$ across both trace decays while the \gls{rtrl} rate does not; the last column is the amplification of exact RTRL over TBPTT(1) at lag $32$.}
\label{tab:weights}
...
what the per-task ceilings in Section~\ref{sec:popgym} are computed from. We did
