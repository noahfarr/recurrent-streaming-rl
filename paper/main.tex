...
\begin{abstract}
We show that real-time recurrent learning (RTRL) over a diagonal recurrence composes with the eligibility trace of a streaming reinforcement learning update, so that one-step truncation decays credit at the trace decay alone while RTRL decays at the slower of that and the recurrence's retention. Streaming methods learn from one transition at a time with no replay buffer, which has kept partially observable tasks out of reach, since truncated backpropagation collapses to a one-step horizon in that setting and RTRL for general recurrent networks is quartic in the hidden size. Restricting to recurrences with a diagonal state-to-state Jacobian, satisfied by RTUs, minGRUs or any other diagonal state-space model, makes RTRL linear in the parameter count and lets it slot into any streaming algorithm built on a per-step gradient and an eligibility trace. Measured on trained agents, lowering the trace decay moves the truncated weight's decay rate and leaves the RTRL weight's where retention holds it. On trace conditioning and MemoryChain RTRL tracks horizons up to four times longer than one-step truncation, and under the intentional update it solves chain length 256 at every trace decay we test, including the standard $0.8$ where the eligibility trace spans five steps. On eleven POPGym tasks the recurrence earns its place on six to nine depending on the algorithm, while recurrent PPO, which uses the same recurrence without the streaming constraint, is matched on masked MuJoCo and stays ahead on eight of eleven, where reusing each transition pays. Neither the trace decay nor the streaming step-size rules transfer from fully observed work, because all three rules we examine assume the eligibility trace is a decayed sum of recent gradients, which the sensitivity violates.
\end{abstract}
...
The evidence is of three kinds, namely credit weights measured on trained agents, which track $\gamma\lambda$ under truncation and the retention of the recurrence under exact \gls{rtrl}, credit horizons on trace conditioning (Figure~\ref{fig:trace}) and MemoryChain that reach four times further than TBPTT(1), and control on POPGym, masked MuJoCo and continual foraging against recurrent \gls{ppo}. We do not claim state-of-the-art performance on any single benchmark.
...
\begin{itemize}
    ...
    algorithms, with a staleness bound for the sensitivity matrix under streaming updates in
    Appendix~\ref{sec:staleness-proof}.
\end{itemize}
...
We evaluate the method on four benchmarks chosen to isolate distinct properties, with two more in the appendices.
...
Continual foraging (Appendix~\ref{sec:foragax}) tests relearning under a periodically inverted reward, where the flips can be absorbed either by relearning the weights or by carrying the phase in the recurrent state. The post-flip minima separate the two, since the memoryless agent dips at every flip while the \gls{rtu} dip shrinks from $0.70$ at the first flip to $1.64$ at the ninth, so the recurrence buys a shallower recovery rather than a higher ceiling.
...
grids and the per-cell gains, and Appendix~\ref{sec:frozen-control} the frozen control.

\subsection{POPGym}

The recurrence earns its place on six to nine of eleven tasks depending on the algorithm, and the streaming methods trail recurrent \gls{ppo} on eight of eleven, where reusing transitions pays. Masked MuJoCo reaches parity once the trace decay is retuned for partial observability.
\label{sec:popgym}
...
stream RAC($\lambda$), from $+160$ to $+973$, and on four of eight for intentional RAC($\lambda$),
...
per-cell intervals. We therefore claim parity rather than an advantage. That claim is only as
good as its baseline, so we bracket the one \gls{ppo} setting left at its published value while
the streaming trace decay was tuned. At $2$M frames with 10 seeds the default GAE $\lambda$ of
$0.95$ reaches $1121$ on masked Hopper against $927$ at $0.9$ and $1112$ at $0.99$, and $1164$ on
masked Walker2d against $1026$ at $0.9$, so it sits at the top of the bracket rather than below
it.
...
Partial observability couples two requirements that are easily conflated, since an agent must
both retain the cue in its state and propagate credit back to the parameters that stored it.
\citet{ni2023transformers} separate the two and find that sequence models can supply memory while
leaving credit assignment largely untouched, and \citet{lafuente2025temporal} quantify how far
back an agent's memory is actually used. Proposition~\ref{prop:composition} speaks to the second
requirement, since retention enters it only through $\rho$, and
Section~\ref{sec:memory_chain} finds the two coming apart in the same direction, with a credit
horizon that exceeds the solvable chain in every cell we test.

Recurrence is the standard remedy for partial observability, and \citet{ni2022recurrent} give
the baseline comparison our masked-MuJoCo protocol follows. The memoryless control we report on
every benchmark follows the logic of memory-improvable domains \citep{tao2025pobax}, where a task
earns its place only if memory demonstrably helps, and POPGym \citep{morad2023popgym} with its
successors \citep{wang2026popgymarcade} supplies the discrete long-memory tasks. Our
continual-foraging evaluation sits in the tradition of tracking rather than converging
\citep{sutton2007tracking, khetarpal2022towards}, where the failure mode is loss of plasticity
\citep{dohare2024loss, abbas2023loss}, which Foragax \citep{tang2026forager} induces with a
periodically flipping reward.
...
The practical consequence is that the constants do not transfer. A streaming algorithm reaching
for a recurrence inherits a trace decay and a step-size rule chosen on fully observed control,
where no part of the network integrates an observation stream, and both assumptions fail once
\gls{rtrl} supplies the gradient. The trace decay has to be set against the credit horizon rather
than copied, which is worth a factor of $5.9$ on masked Hopper, and the step-size rules rest on
the trace being a decayed sum of recent gradients, which the sensitivity violates in three
separate ways. Neither correction is specific to the algorithms we test, so both should apply to
any streaming update built on the same two ingredients.

What the evidence does not support is a claim that recurrence alone is enough. Retention is
necessary but not sufficient, since the credit horizon exceeds the solvable chain in every
MemoryChain cell, by factors from $1.5$ to nearly $400$, so what limits these agents at long
horizons is something the bound does not capture. The streaming agents also trail recurrent
\gls{ppo} on eight of eleven POPGym tasks, three of which no method beats its memoryless
counterpart, which is the cost of one pass over each transition rather than many. Both
intentional variants diverge until their preconditioner is corrected, and on continual foraging
the best agent under that update is memoryless.

Three limitations are structural. Every agent here carries a single recurrent layer, so we cannot
say how the horizon composes through depth. Masked MuJoCo is the one benchmark we did not run
with a second recurrence, leaving the cell and the task confounded there. And ten seeds resolve
the differences we report but not the narrow ones, which we mark as unresolved rather than
settle.
...
$n{=}32$ for minGRU against $6$ to $9$ for \gls{rtu}, and the cause is phase cancellation, since
...
\gls{rtu}, which begins at $0.997$. And exploration cuts favour \gls{rtrl}, since both implementations
...
sufficiency, since the credit horizon exceeds the observed solvable chain in every cell of every grid.
...
\eqref{eq:amplification}, since at $\lambda{=}0.98$ the exponential form
...
draw no regime distinction from it. What is robust is its size, since exact \gls{rtrl}
...
\section{Continual foraging}
\label{sec:foragax}

A memoryless agent relearns after every flip, and the recurrent state takes the flips
over.
...
\begin{figure}[htbp]
    ...
    \gls{rtu}, namely the reward rate, an exponential moving average that tracks current performance,
...
trace mass has recently carried gradient mass. Exact \gls{rtrl} breaks the assumption, since the
...
\gls{rtrl} the trace is dense and that norm grows through training, so on masked Hopper the
...
unaffected. The mass is also badly distributed, since measured on the masked-Hopper actor the
...
saw them. The symptoms follow, since on masked Hopper the actor step is sixteen times smaller than
...
with \gls{rtu} is otherwise dead from $700$k frames onward and recovers to a reward rate of
...
acute. On masked MuJoCo the same change removes the divergence outright, and intentional RAC($\lambda$)
...
one. Where the original is stable it costs performance, since on six POPGym tasks it is worse on every
...
should not, since $\lambda = 1$ brings $\gamma\lambda$ closer to $\rho$ than $\lambda = 0.99$ does
...
observation stream. \gls{ppo}'s GAE $\lambda$ stays at the $0.95$ of \citet{elelimy2024real},
which Section~\ref{sec:mujoco} brackets on both sides and finds to be the top of the bracket.
...
with the trace, so it pays for the extra length twice, and on masked Hopper its actor step falls
...
setting. Intentional RAC($\lambda$) does not, and peaks at $\eta_\pi = 0.01$, five times smaller
...
task rewards accumulation, since on MultiArmedBandit a minGRU under stream RQ($\lambda$) reaches $0.76$
...
what the per-task ceilings in Section~\ref{sec:popgym} are computed from. Continual foraging reports the same
three recurrences in Appendix~\ref{sec:foragax}. We did not run masked MuJoCo with a second
...
Section~\ref{sec:memory_chain}, namely RQRC($\lambda$) with TBPTT(1) on a cell whose recurrent
...
hyperparameters reported by \citet{elelimy2024real}, including their real-time update, namely
...
(Section~\ref{sec:memory_chain}). Stream RQ($\lambda$) and intentional RQ($\lambda$) use the same
values apart from the step-size rule each brings from its own work, and the last three rows are
...
MLP width                              & $64$ \\
minGRU hidden dimension                & $192$ \\
\midrule
Chain length, $L$                      & $\{16, 32, 64, 96, 128, 192, 256\}$ \\
Trace decay, $\lambda$                 & $\{0.8, 0.9, 0.98, 0.995, 1\}$ \\
Gradient computation                   & TBPTT(1), exact \gls{rtrl} \\
\bottomrule
\end{tabular}
\end{table}
...
(Appendix~\ref{sec:foragax}). Stream RQ($\lambda$) and intentional RQ($\lambda$)
...
\end{figure}
  ...
  computation, where each network maintains an RTU hidden state $\boldsymbol{h}$ and a
...
$\boldsymbol{\omega}_k^{\mathrm{TBPTT(1)}}$, since exact \gls{rtrl} adds the contributions
...
upper bounds throughout, since $\|\boldsymbol{\Phi}_{i,k}\| \le \rho^{\,i-k}$ gives no matching lower
...
substitute for the recurrent gradient, and does so only when the intermediate
...
The two algorithms differ in absolute staleness level, and stream RAC($\lambda$)
...
\gls{rtu} layer incurs a \emph{staleness error}, since immediate Jacobians
...
than diverging. The two factors of $T$ have distinct origins, where one counts the
