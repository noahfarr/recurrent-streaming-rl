...
\begin{figure}[t]
    ...
    \caption{Where the composition predicts exact \gls{rtrl} helps, and where it does not. Gain in
    solve rate from exact \gls{rtrl} over TBPTT(1) on MemoryChain, as a
    function of chain length $L$ and trace decay $\lambda$, for three streaming algorithms
    with a minGRU recurrence ($\gamma{=}0.999$, $16$--$32$ seeds per cell). A run counts as
    successful when its final return exceeds $0.5$. White cells mark combinations where
    neither method solves the task, distinguishing them from cells where both already succeed
    and the difference is likewise zero. Per-method solve rates for all three sweeps are given
    in Appendix~\ref{sec:memory-chain-grids}.}
...
\begin{itemize}
    ...
    triangular system whose slower mode sets the credit horizon, verified by measuring credit
    weights on trained agents to within two percent.
    \item The consequence that horizon is not shared: parameters reached by the sensitivity
    ...
    hundreds of steps. Every task we sweep shows an interior optimum, worth a factor of $5.9$
    on masked Hopper, but its location is task-dependent and does not track $\rho$
    (Appendix~\ref{sec:lambda}).
    \item The observation that streaming step-size rules assume the trace is a decayed sum of
    recent gradients, which \gls{rtrl} violates, and that this breaks them in three distinct
    ways; with trace preconditioning, which recovers a configuration that otherwise
    diverges and helps memoryless agents too (Appendix~\ref{sec:stepsize-analysis}).
    \item Streaming deep RL for partial observability with exact \gls{rtrl} over any
    diagonal-Jacobian recurrence, instantiated with \gls{rtu}s and minGRUs in four streaming
    algorithms, evaluated on trace conditioning, MemoryChain, POPGym, masked MuJoCo and
    continual foraging against recurrent \gls{ppo} at matched capacity.
    \item A staleness bound for the \gls{rtrl} sensitivity matrix under streaming updates with
...
\gls{rtu}s \citep{elelimy2024real} sidestep this barrier with a linear diagonal complex-valued recurrence (their linear variant, with a nonlinearity only on the output),
\begin{equation}
...
$\lambda_k = r_k(\cos\Omega_k + i\sin\Omega_k)$, where $r_k$ and $\Omega_k$ define the polar complex representation of the $k$'th parameter. Because each hidden unit
has only one recurrent connection to itself, exact \gls{rtrl} has linear time and memory complexity in the number of
parameters, which Figure~\ref{fig:rtrl} in Appendix~\ref{sec:memory-chain-extra} confirms empirically.

The minGRU \citep{feng2024were} satisfies the same condition through a real gate rather than a
...
$\boldsymbol{J}_t = \mathrm{diag}(\boldsymbol{1}-\boldsymbol{u}_t)$ and exact \gls{rtrl} is again
linear in the parameter count. The two differ in where retention comes from, \gls{rtu}'s $r_k$
...
mapping observations to values or actions, we insert an \gls{rtu} layer between the observation and the feedforward head. The streaming update machinery, including eligibility traces and step-size adaptation, is unchanged. The only modification is how the per-step gradient that feeds into the eligibility
trace is computed. Figure~\ref{fig:architecture} shows the data flow, and we describe the gradient computation for a single
network below.
...
$\phi(\,\cdot\,;\boldsymbol{\theta})$ producing features $\boldsymbol{x}_t$, a recurrent \gls{rtu}
layer with parameters $\boldsymbol{\psi}$ producing the state
$\boldsymbol{h}_t$, and a feedforward head with parameters $\boldsymbol{w}$
(a value head $v(\boldsymbol{h}_t; \boldsymbol{w}_v)$, say). We compute the gradient
of the head output with respect to each group in a single backward pass,
combined with the \gls{rtu}'s forward-mode \gls{rtrl} trace
...
$\mathcal{O}(|\boldsymbol{\psi}|)$ per step. The encoder parameters are
more complicated. They influence the state both instantaneously through
$\boldsymbol{x}_t$ and recursively through every past state. Maintaining the exact
trace would cost $\mathcal{O}(|\boldsymbol{\theta}|\cdot\dim \boldsymbol{h})$ per step,
prohibitive for a deep encoder. We therefore use a one-step
approximation, backpropagating through the head, a single step of the
recurrence, and the encoder. This credits the encoder for its immediate
effect on $\boldsymbol{h}_t$ while ignoring its influence through past
states.

\paragraph{Composition with eligibility traces.}
Operationally the two traces compose without modification. The \gls{rtrl}
trace is a forward-mode object that maintains
$\partial \boldsymbol{h}_t / \partial \boldsymbol{\psi}$ at each step; the eligibility
trace is a temporal accumulator of whatever per-step gradient the streaming
algorithm supplies. Each \gls{rtu} layer supplies its
$\partial \boldsymbol{h}_t / \partial \boldsymbol{\psi}$ on demand whenever the streaming
update needs the gradient with respect to its recurrent parameters, and the
eligibility trace folds this into its usual recursion.

Their effects on credit assignment, however, are not independent. Both carry credit backward in
time, and the reach of the update is set by whichever decays more slowly. The bound governs how
...
Let $\boldsymbol{\Phi}_{i,k} \doteq \boldsymbol{J}_i \boldsymbol{J}_{i-1}\cdots\boldsymbol{J}_{k+1}$
for $i > k$, with $\boldsymbol{\Phi}_{k,k}$ the identity matrix. At the end of an episode of
length $T$, both updates are weighted sums over the same immediate Jacobians,
$\boldsymbol{z}_T = \sum_{k=0}^{T} \boldsymbol{\omega}_k \boldsymbol{I}_k$, with
\begin{equation}
...
\end{equation}
Writing $\rho \doteq \max_t \|\boldsymbol{J}_t\|$ and $C \doteq \max_t \|\boldsymbol{c}_t\|$,
\begin{equation}
    \|\boldsymbol{\omega}_k^{\mathrm{\gls{rtrl}}}\|
...
Substituting the \gls{rtrl} recursion into the eligibility trace shows why:
...
between them, so the decay is set by whichever mode is slower. Writing $n = T-k$ for the lag,
...
by its last otherwise. Exact \gls{rtrl} therefore amplifies distant credit linearly or
exponentially in the lag, and offers no advantage in decay rate only when
$\rho < \gamma\lambda$. We measure the rates directly below.

The bound governs how credit is weighted, not whether any is available to assign. Appendix~\ref{sec:composition-caveats} discusses three qualifications, including why $\lambda$ is not free and why $\rho > \gamma\lambda$ is the typical case.


\paragraph{The bound is tight enough to measure.}
\label{sec:weights}
Freezing a trained agent and measuring $\|\boldsymbol{\omega}_k\|$ against lag on MemoryChain
($L{=}64$, medians over 10 seeds), the TBPTT(1) weight decays at $\gamma\lambda$ to within
$0.2\%$ in every configuration and the \gls{rtrl} weight at $\max(\gamma\lambda,\rho)$ to within
$2\%$, for both cells, both trace decays and all three value-based algorithms. The amplification
at lag $32$ is cell-dependent, $20$ to $33$ for minGRU against $6$ to $9$ for \gls{rtu}, which
the same measurements attribute to phase cancellation: minGRU's coherence is exactly $1$ at
every lag because its retentions are real and positive, while \gls{rtu}'s complex rotation
drives it to $0.23$--$0.46$ by lag $32$. What the measurements do not establish is sufficiency,
since the credit horizon exceeds the observed solvable chain in every cell of every grid.
Appendix~\ref{sec:credit-appendix} gives the protocol, the fitted rates and the effect of
exploration cuts, which favour \gls{rtrl} because zeroing the eligibility trace on a non-greedy
action leaves the influence matrix intact.

\paragraph{Instantiations.} We evaluate on five streaming algorithms covering prediction,
value-based control and policy-based control: stream TD($\lambda$), stream Q($\lambda$) and
...
We evaluate the method on five benchmarks chosen to isolate distinct properties, and on a sixth in the appendix.
...
KMemoryChain (Appendix~\ref{sec:staleness}) sweeps a memory length to measure the staleness of sensitivities in \gls{rtrl}.
All curves report the interquartile mean (IQM) with shaded standard error, and seed counts vary by experiment and are stated in each caption. The recurrence is not held constant: each benchmark uses the cell from the work it comes from, minGRU for MemoryChain and \gls{rtu} elsewhere, and Appendix~\ref{sec:popgym-cells} repeats POPGym with the other two.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.72\linewidth]{plots/trace_conditioning/return_error.pdf}
...
Trace conditioning \citep{rafiee2022eye} is a prediction task: a conditioned stimulus precedes an unconditioned one by an interstimulus interval (ISI), and the agent observes both and predicts the discounted sum of the unconditioned stimulus, with $\gamma$ set per ISI as in the original benchmark so that $1/(1-\gamma)$ matches the ISI. This is the setting of Proposition~\ref{prop:composition}: prediction under a fixed behaviour, with the memory length set by the ISI. We run stream RTD($\lambda$) with \gls{rtu}, minGRU and a memoryless network for 2M steps and 10 seeds and report the mean squared error of the prediction against the true return.


\paragraph{Result.}
Figure~\ref{fig:trace} shows that \gls{rtu} under exact \gls{rtrl} is the only configuration that tracks the timing beyond ISI 10: at ISI 20 and 40 its error is $7\times$ and $3.4\times$ below TBPTT(1) at $\lambda{=}0.9$, where TBPTT(1) sits near the memoryless baseline. Raising $\lambda$ to $1$ helps TBPTT(1) substantially ($0.18$ to $0.07$ at ISI 20), which is the
eligibility trace extending the horizon. Exact \gls{rtrl} moves less but not negligibly: at ISI
40 its error falls from $0.085$ at $\lambda{=}0.9$ to $0.057$ at $0.95$, rising again to $0.063$
...
parallel and the slower mode dominates. At $\lambda{=}0.8$ and $\gamma{=}0.99$ the trace mode
decays with an effective horizon of roughly five steps, which is consistent with the collapse
observed past $L{=}16$; the recurrent gradient is then the only channel that remains.
Raising $\lambda$ to one removes that limit. Figure~\ref{fig:memory_chain_lambda} sweeps both
quantities jointly under otherwise identical settings ($\gamma{=}0.999$, minGRU recurrence,
$16$--$32$ seeds per cell) for three streaming algorithms. Section~\ref{sec:weights} measures
...
$\lambda{=}0.995$, though sublinearly: the ratio $L/(1-\gamma\lambda)^{-1}$ falls from $3.2$ to
$0.77$ across the sweep. Exact \gls{rtrl} moves that boundary one grid step deeper at every
$\lambda$ but one, and its advantage is largest along the boundary, reaching $0.69$ at
$L{=}32,\lambda{=}0.8$. The advantage vanishes in two limits the figure keeps distinct: at high
$\lambda$ the trace already spans the episode and the two channels coincide, while at long $L$
neither method succeeds. We do not attribute the latter to the recurrence losing the cue, since
the same minGRU solves $L{=}256$ under the intentional update at every $\lambda$.

The ordering is algorithm-independent and the reachable horizon is not. Exact \gls{rtrl} is at
least as good as TBPTT(1) in every cell of all three grids but one, with mean gains of $+0.14$,
$+0.13$ and $+0.25$, while the absolute reach differs by an order of magnitude: stream
...
are. It is not
sufficient, however: the credit horizon $(1-\rho)^{-1}$ exceeds the observed boundary in every
...
The recurrent agents beat the identical updates without a recurrence on seven of eleven tasks, and trail recurrent \gls{ppo} on eight of eleven, where reusing transitions pays. Masked MuJoCo reaches parity once the trace decay is set for the recurrence.
\label{sec:popgym}
...
of an optimal policy, which puts the panels on a common scale. On HigherLower, Concentration and
MultiArmedBandit the optimal return is below the top of the return range, so normalising to the
range instead would understate how close the agents come.
Every curve begins at a measured value rather than an extrapolation. We evaluate
the freshly initialized policy with no updates applied, so the leftmost point is
the return of the untrained agent.
...
identical update with the recurrence removed rather than against an estimate of optimal reactive
behaviour. A solid curve above its own dashed one means the recurrence earns its place on that
task.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.76\linewidth]{plots/popgym/learning_curves.pdf}
...
\paragraph{Result.}
The ceiling partitions the suite. On five of the eleven tasks no streaming method clears it:
HigherLower is reached exactly at the ceiling, Autoencode stays within $0.03$ of initialisation,
and on MultiArmedBandit, Battleship and Concentration the best streaming method reaches $0.63$,
$0.44$ and $0.42$ against ceilings of $0.99$, $0.63$ and $0.80$. Recurrent \gls{ppo} clears
Autoencode and none of the other four. On these five the agents fail to recover
behaviour a reactive baseline already implements, so they say little about recurrent
architectures. On the remaining six at least one streaming method exceeds it: RepeatPrevious,
StatelessCartPole, Minesweeper and NoisyStatelessCartPole are cleared by all four methods, and
CountRecall and RepeatFirst by every method except RQRC($\lambda$).

Against recurrent \gls{ppo} the streaming methods do not reach parity. They finish ahead on three tasks, all of them recall, where a single pass over a long-horizon dependency suffices: RepeatPrevious ($0.99$ against $0.81$), RepeatFirst ($0.48$ against $0.31$) and CountRecall ($0.23$ against $0.10$). They finish behind on the other eight, narrowly on Minesweeper, Autoencode and HigherLower, where every method sits at the ceiling, and by a wide margin on Concentration ($0.42$ against $0.78$), MultiArmedBandit ($0.63$ against $0.81$) and both cartpole variants, where reusing each transition many times pays. Stream RQ($\lambda$) and intentional RQ($\lambda$) part company on Battleship ($0.03$ against $0.42$) and RepeatPrevious ($0.43$ against $0.99$), and RQRC($\lambda$) is the weakest.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\linewidth]{plots/mujoco/learning_curves.pdf}
    \caption{IQM episodic return with standard error, 10 seeds, on four MuJoCo tasks under masked velocities (\textbf{P}) and masked positions (\textbf{V}). Streaming methods use $\lambda = 0.99$, and \gls{ppo} uses the same network as the streaming agents (Appendix~\ref{sec:hyperparameters}). Curves are drawn only where all 10 seeds survive, so the intentional RAC($\lambda$) curves end where its updates become non-finite.}
    \vspace{-0.3cm}
    \label{fig:brax}
\end{figure}
...
benchmark (Figure~\ref{fig:brax}). Bootstrapping the difference of means over the 10 seeds per cell ($20{,}000$
resamples, $95\%$ percentile interval) separates only three of eight: \gls{ppo} is ahead on
Ant-V and Walker2d-P, stream RAC($\lambda$) on Hopper-V, and the other five overlap, including
the three where the point estimates favour us most. Appendix~\ref{sec:stepsize} lists the
per-cell intervals. We therefore claim parity rather than an advantage, and note that ten seeds
is too few to resolve differences of this size.

Most of the difference from the setting these methods ship with is the trace decay: at
$\lambda = 0.8$ stream RAC($\lambda$) reaches $262$ on Hopper-P against $1550$ at $0.99$, a
factor of $5.9$ (Appendix~\ref{sec:lambda}). Walker2d is the one task that responded to no
intervention we tried, neither $\lambda$, nor a per-block step size, nor removing the entropy
bonus, nor matching the network size (Appendix~\ref{sec:stepsize}).

Intentional RAC($\lambda$) does not complete the benchmark as published: on three cells its
...
estimating it from the trace instead removes the divergence, after which all eight cells run to
5M on every seed and seven of eight improve, Ant-P from $1276$ to $1946$. It still trails stream
RAC($\lambda$) everywhere, so we report it as a method that now completes the benchmark rather
than a competitive one.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.92\linewidth]{plots/foragax/foragax_body.pdf}
    ...
    Right compares the recurrences within stream RQ($\lambda$): the minimum reward rate reached
...
between $1.2$ and $1.4$ at every flip, the cost of relearning in the weights, and \gls{ppo} to
...
higher ceiling, \gls{rtu} and the FFN finishing at $1.73$ and $1.63$ with a difference of
$[-0.42, 0.40]$, and under intentional RQ($\lambda$) the memoryless
...
What we did not achieve is worth stating together. The streaming agents trail recurrent
\gls{ppo} on eight of eleven POPGym tasks, five of which no method beats its memoryless
counterpart. Walker2d resisted every intervention, both intentional variants diverge
until their preconditioner is corrected, and on Foragax the best agent under that update is
memoryless. Retention long enough to span the dependency is necessary but not sufficient, since on
MemoryChain the credit horizon exceeds the solvable chain everywhere. Two limitations are
structural, a single recurrent layer and ten seeds per cell.
...
\begin{table}[H]
\centering
\caption{Decay of the credit weight $\|\boldsymbol{\omega}_k\|$ with lag on trained MemoryChain agents ($L{=}64$, medians over $10$ seeds) against the rates of Proposition~\ref{prop:composition}: $\gamma\lambda$ for TBPTT(1) and $\max(\gamma\lambda, \rho)$ for RTRL, with $\rho$ the measured retention. Both fitted rates match to within $2\%$, and the last column is the amplification of exact RTRL over TBPTT(1) at lag $32$.}
\label{tab:weights}
\begin{tabular}{llllccc}
\toprule
Cell & $\lambda$ & $\gamma\lambda$ & $\rho$ &
$\|\boldsymbol{\omega}^{\mathrm{TBPTT(1)}}\|^{1/n}$ &
$\|\boldsymbol{\omega}^{\mathrm{\gls{rtrl}}}\|^{1/n}$ & Ratio at $n{=}32$ \\
\midrule
minGRU & $1$    & $0.9990$ & $0.9993$ & $0.9995$ & $1.0140$ & $20.1$ \\
minGRU & $0.98$ & $0.9790$ & $0.9995$ & $0.9791$ & $0.9942$ & $33.2$ \\
\gls{rtu}    & $1$    & $0.9990$ & $0.9989$ & $0.9972$ & $1.0193$ & $6.4$ \\
\gls{rtu}    & $0.98$ & $0.9790$ & $0.9989$ & $0.9786$ & $1.0007$ & $9.2$ \\
\bottomrule
\end{tabular}
\end{table}
...
case, confirming the first weight of Proposition~\ref{prop:composition} directly.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{plots/rtrl.pdf}
        \caption{Per-step wall-clock time (ms) of a single gradient update as a function of hidden width, comparing feedforward backpropagation (FFN) against exact \gls{rtrl} for \gls{gru}, LSTM, and \gls{rtu} recurrent layers.}
        \label{fig:rtrl}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{plots/memory_chain/interval_estimates.pdf}
        \caption{Episodic return as a function of chain length on the MemoryChain diagnostic, averaged over 5 seeds. We compare a feedforward network, GRU and RTU with TBPTT over 1 step and RTU with RTRL.}
        \label{fig:memory_chain}
    \end{subfigure}
\end{figure}

\paragraph{Chain-length sweep at a single trace decay.}
Figure~\ref{fig:memory_chain} reports IQM episodic return against $L$.
At $\lambda{=}0.8$ the three TBPTT(1) curves drop to near-zero return past $L{=}16$ and stay
there for the remainder of the sweep, while the \gls{rtu}-\gls{rtrl} curve holds near the
maximum return through $L{=}48$ and degrades through $L{=}64$.
Since both \gls{rtu} agents use the same recurrence and the same streaming update, and
differ only in the computation of $\partial \boldsymbol{h}_t / \partial \boldsymbol{\psi}$,
the gap between the dotted and solid curves is attributable to the gradient method.
...
ISI 20 is $0.085$ at $\lambda = 0.9$, $0.033$ at $0.95$ and $0.047$ at $0.99$, and at ISI 40
$0.085$, $0.057$ and $0.063$, so $0.95$ is best and $0.99$ is worse. On four POPGym recall tasks
$\lambda = 0.99$ is worse than the $0.95$ we report throughout, and substantially so:
RepeatPrevious falls from $0.99$ to $0.35$, RepeatFirst from $-0.07$ to $-0.48$ and CountRecall
from $-0.53$ to $-0.96$. What generalises is that an interior optimum exists and that it sits
...
sharpens, so $\|z\|_1$ grows and the step falls over training. If that shrinking step were
the cause of the gap to recurrent \gls{ppo} in Section~\ref{sec:mujoco}, raising the step
would recover it. It does not. Table~\ref{tab:stepsize} sweeps the actor constant of both
methods on the two cells where the gap is widest, at 2M frames with 3 seeds, against the
default read from the main 10-seed runs at the same frame count. Every setting that raises
the step above the default leaves both methods no better and usually much worse, and the
one nominal gain, stream RAC($\lambda$) at $\kappa_\pi = 1$ on Hopper-P, has a standard
error of $161$ over 3 seeds and remains far below the $1375$ that recurrent \gls{ppo}
reaches at the same budget. The $\kappa_\pi$ rows also set $\kappa_V = 1$ rather than the
default $2$, so they bound the effect of the pair rather than of $\kappa_\pi$ alone.

\begin{table}[H]
\centering
\caption{Final episode return at 2M frames on masked MuJoCo under actor step-size sweeps,
mean over 3 seeds with standard error. Default rows are the main 10-seed runs read at 2M
frames.}
\label{tab:stepsize}
\begin{tabular}{llrr}
\toprule
...
\midrule
stream RAC($\lambda$)      & $\kappa_\pi = 3$ (default) & $223 \pm 7$   & $749 \pm 103$ \\
                           & $\kappa_\pi = 1$           & $393 \pm 161$ & $620 \pm 114$ \\
                           & $\kappa_\pi = 0.3$         & $80 \pm 55$   & $667 \pm 20$ \\
                           & $\kappa_\pi = 0.1$         & $4 \pm 1$     & $24 \pm 7$ \\
\midrule
intentional RAC($\lambda$) & $\eta_\pi = 0.05$ (default) & $431 \pm 94$ & $613 \pm 61$ \\
                           & $\eta_\pi = 0.2$            & $264 \pm 70$ & $303 \pm 50$ \\
                           & $\eta_\pi = 1$              & $116 \pm 51$ & $102 \pm 86$ \\
                           & $\eta_\pi = 5$              & $12 \pm 5$   & $29 \pm 25$ \\
\midrule
recurrent \gls{ppo}        & ---                         & $1375 \pm 68$ & $980 \pm 77$ \\
\bottomrule
\end{tabular}
\end{table}
...
The recurrence is not held constant across benchmarks: MemoryChain uses minGRU, POPGym and
masked MuJoCo use \gls{rtu}, and trace conditioning reports both. We chose each from the cell
that the corresponding source paper used, not from performance on the benchmark, but the choice
matters: Section~\ref{sec:memory_chain} reports that at $\lambda{=}1$ and $L{=}128$ a minGRU
solves $60\%$ of seeds where an \gls{rtu} solves none. Figures~\ref{fig:popgym-mingru}
...
Section~\ref{sec:popgym}. The ordering among algorithms is preserved and the memoryless
comparison is what the per-task ceilings in Section~\ref{sec:popgym} are computed from. We did
not run masked MuJoCo or Foragax with a second recurrence, so the cell and the benchmark remain
confounded there.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\linewidth]{plots/popgym/learning_curves_mingru.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\linewidth]{plots/popgym/learning_curves_ffn.pdf}
...
\begin{table}[H]
  \centering
  \caption{Hyperparameters for stream AC$(\lambda)$ on POPGym (Section~\ref{sec:popgym}).}
  \label{tab:hparams-popgym-streamac}
  \begin{tabular}{ll}
  \toprule
  Name & Value \\
  \midrule
...
Critic $\kappa_V$                      & $2.0$ \\
\midrule
Intentional RAC($\lambda$) optimizer   & intentional update \\
Actor $\eta_\pi$                       & $0.05$ \\
Critic $\eta_V$                        & $0.5$ \\
\midrule
